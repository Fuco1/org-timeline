...
(defcustom org-timeline-dedicated-clocked-line t
  "Option to show clocked items in a dedicated line titled '$'."
  :type 'boolean
  :group 'org-timeline)

(defcustom org-timeline-overlap-in-new-line nil
  "Option to create new lines when blocks overlap."
  :type 'boolean
  :group 'org-timeline)

(defcustom org-timeline-emphasize-next-block nil
  "Option to apply the face `org-timeline-next-block' face to the next block happening today."
  :type 'boolean
  :group 'org-timeline)
...
(defvar org-timeline-first-line-in-agenda-buffer 0
  "First line of the timeline in the agenda buffer.")
...
(cl-defstruct org-timeline-task
  ...
  beg  ; beginning of task in day (in minutes)
  end  ; end of task in day (in minutes)
  offset-beg ; beginning of block in timeline line
  offset-end ; end of block in timeline line
  info ; info line for the corresponding task
  line-in-agenda-buffer ; line number where this task is displayed in the agenda buffer
  face ; the task block's face
  day  ; day of the task (absolute, see `calendar-absolute-from-gregorian')
  type ; type of the task ("scheduled", "clocked" ...)
  text ; the text to display inside the block
  category  ; category shown before the block's timeline line.
...
(defface org-timeline-next-block
  '((t (:background "dark olive green")))
   "Face used for the next block happening today.
          ...
          (let* ((hour (/ time-of-day 100))
            ...
            (when (and (>= beg start-offset)
              ...
              (push (make-org-timeline-task
                     ...
                     :category (org-timeline--get-category type)
...
Return t if this task will overlap another one when inserted."
  (let* ((slotline (org-timeline--add-elapsed-face org-timeline-slotline))
         ...
         (category (org-timeline-task-category task))
      ...
      (insert (concat "\n" ;; creating the necessary lines, up to the current task's day
                      (mapconcat (lambda (line-day)
                                   (propertize (concat (calendar-day-name (mod line-day 7) t t) ;; found in https://github.com/deopurkar/org-timeline
                                                       " "
                                                       slotline)
                                               'org-timeline-day line-day 'org-timeline-category "   "))
    ...
    (while (and (not (string= (get-text-property (point) 'org-timeline-category) category))
                (eq (get-text-property (point) 'org-timeline-day) day))
      (forward-line))
    (unless (string= (-if-let (cat-here (get-text-property (point) 'org-timeline-category)) cat-here "   ") category)
      (when (not (eq (line-end-position) (point-max))) (forward-line -1))
      (goto-char (line-end-position))
      (insert "\n"
              (propertize (concat category " " slotline) 'org-timeline-day day 'org-timeline-category category)))
    ...
    (while (org-timeline--new-overlap-line-required-at-point-p task)
      (let ((decorated-slotline (propertize (concat category " " slotline)
                                            'org-timeline-day day
                                            'org-timeline-category category)))
        (if (eq (forward-line) 1) ;; reached end or buffer
            (insert (concat "\n" decorated-slotline))
          (when (not (eq (get-text-property (point) 'org-timeline-category) category)) ; reached end of category's section
...
Move tomorrow's line to the right of today's line, in order to show a complete 24h cycle.
